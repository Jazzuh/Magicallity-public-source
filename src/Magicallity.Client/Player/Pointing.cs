...
{
    class Pointing : ClientAccessor
    {
        private bool isPointing = false;

        public Pointing(Client client) : base(client)
        {
        ...
        private async Task OnTick()
        {
            ...
            if (Input.IsControlJustPressed(Control.SpecialAbilitySecondary))
            {
                if (Cache.PlayerPed.IsDoingAction()) return;
