...
namespace Magicallity.Client.UI
{
    internal class CharacterEditorCamera
    {
        ...
        public CharacterEditorCamera(bool useCameras = true)
        {
            ...
            headCam = World.CreateCamera(new Vector3(pos.X += 0.3f, pos.Y += 1.0f, pos.Z), new Vector3(180, 180, -20), 45);

            pos = Game.PlayerPed.Bones[Bone.SKEL_Spine3].Position;
            bodyCam = World.CreateCamera(new Vector3(pos.X += 0.3f, pos.Y += 1.0f, pos.Z), new Vector3(180, 180, -20), 75);
            calculateCameraHeights(Game.PlayerPed.Bones[Bone.SKEL_Spine3]);

            pos = Game.PlayerPed.Bones[Bone.SKEL_R_Calf].Position;
            LegCam = World.CreateCamera(new Vector3(pos.X += 0.3f, pos.Y += 1.0f, pos.Z), new Vector3(180, 180, -20), 75);
