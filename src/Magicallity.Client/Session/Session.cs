...
namespace Magicallity.Client.Session
{
    public class Session
    {
        ...
        {
            dataUpdated = false;
            Magicallity.Client.Client.Instance.TriggerServerEvent("Session.UpdateClientData", updateData, $"{callingMember} in {fileName.Split('\\').ToList().Last()} on line {lineNumber}");
            while (!dataUpdated)
            {
        ...
        {
            canPayForItem = null;
            Magicallity.Client.Client.Instance.TriggerServerEvent("Payment.CanPayForItem");
