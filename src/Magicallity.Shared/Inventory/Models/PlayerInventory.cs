...
using static Magicallity.Shared.InventoryItems;
#if SERVER
...
namespace Magicallity.Shared.Models
{
    public sealed class PlayerInventory : Inventory
    {
        ...
        public override void AddItem(inventoryItem item, int amount, Player userSource = null, [CallerMemberName] string callingMember = "", [CallerFilePath] string fileName = "", [CallerLineNumber] int lineNumber = 0)
        {
            ...
            if (amount >= 0)
            {
                ...
                //Log.Debug($"[{Log.getDateString(DateTime.Now)}] " +
                    //$"{sourcePlayer.Name} (steam:{sourcePlayer.Identifiers[Magicallity.Server.Server.CurrentIdentifier]}) obtained {amount} {invItem.itemName} // Called via: {callingMember} in {callingLocation} on line {lineNumber}");
            }
            else
            {
                ...
                //Log.Debug($"[{Log.getDateString(DateTime.Now)}] " +
                 //           $"{sourcePlayer.Name} (steam:{sourcePlayer.Identifiers[Magicallity.Server.Server.CurrentIdentifier]}) disposed of {amount * -1} {invItem.itemName} // Called via: {callingMember} in {callingLocation} on line {lineNumber}");
            }
        ...
        /// <param name="itemKey">Item name or item code of the <see cref="inventoryItem"/> wanting to be used</param>
        /// <param name="itemAmount">Amount of the item wanting to be used</param>
        public void UseItem(string itemKey, int itemAmount) => Magicallity.Client.Client.Instance.TriggerServerEvent("Inventory.UseInvItem", itemKey, itemAmount);
#endif
    }
}
