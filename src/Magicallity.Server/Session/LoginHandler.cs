...
using Magicallity.Server.MySQL;
...
namespace Magicallity.Server.Session
{
    class LoginHandler : ServerAccessor
    {
        ...
        private async void OnCreateRequest([FromSource] Player source, string firstName, string lastName, string dateOfBirth)
        {
            ...
            if (canUseName)
            {
                MySQL.execute("INSERT INTO character_data (`SteamID`, `FirstName`, `LastName`, `DOB`, `Settings`) VALUES (@steam, @first, @last, @dob, @settings)", new Dictionary<string, dynamic>
                {
                    ["@steam"] = source.Identifiers[Server.CurrentIdentifier],
                    ["@first"] = firstName,
                    ["@last"] = lastName,
