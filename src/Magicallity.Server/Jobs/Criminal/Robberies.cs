...
using Magicallity.Shared.Locations;
        ...
        private void OnRobRequest([FromSource] Player source, string bank)
        {
            ...
            if (!robberyModel.IsBeingRobbed && robberyModel.IsRobbable && copsOnDuty.Count >= robberyModel.RequiredPolice)
            {
                if(robberyModel.IsBank)
                {
                    ...
                    if (playerSession.GetLocalData("Bank.CanRobVault", false) && robberyModel.IsVault || !robberyModel.IsVault)
                    {
...
#pragma warning restore 4014
    ...
    public class RobberyModel
    {
        ...
        public int RequiredPolice => CitizenFX.Core.Native.API.GetConvarInt($"mg_{LocationName}RequiredPolice", IsBank ? 5 : 4);
        private int MaxPayout => CitizenFX.Core.Native.API.GetConvarInt($"mg_{LocationName}RobberyPayout", IsBank ? (!IsVault ? 1000 : 75000) : 10000);
        private Random rand => new Random((int)DateTime.Now.Ticks);
        public int Payout => rand.Next(MaxPayout / 4, MaxPayout);
        public int CooldownTime => CitizenFX.Core.Native.API.GetConvarInt($"mg_{LocationName}CooldownTime", IsBank ? 5400 : 2700) * 1000 / (IsBank ? (!IsVault ? 2 : 1) : 1);
