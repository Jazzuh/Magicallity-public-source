...
using Magicallity.Server.Shared.Enums;
...
using Magicallity.Server.Jobs.EmergencyServices.EMS;
...
namespace Magicallity.Server.Jobs.EmergencyServices.EMS
{
    public class DeathHandler : ServerAccessor
    {
        ...
        /*public DeathState GetDeathState(Session.Session playerSession)
        {
            Enum.TryParse<DeathState>(playerSession.GetGlobalData("Character.DeathState", "Alive"), out var deathState);

            return deathState;
        }

        public void SetPlayerDeathState(Session.Session playerSession, DeathState deathState)
        {
            playerSession.SetGlobalData("Character.DeathState", deathState.ToString());
            playerSession.TriggerEvent(deathState == DeathState.Dead ? "Death.StartDeathThread" : "Death.EndDeathThread");
            if(deathState == DeathState.Dead)
                startDeathTimer(playerSession);
            else
